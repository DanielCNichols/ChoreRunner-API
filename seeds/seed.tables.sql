...
--update the sequence
SELECT setval('tasks_id_seq', (SELECT MAX(id)
  from "tasks"));
SELECT setval('members_id_seq', (SELECT MAX(id)
  from "members"));
SELECT setval('households_id_seq', (SELECT MAX(id)
  from "households"));
SELECT setval('users_id_seq', (SELECT MAX(id)
  from "users"));
SELECT setval('levels_id_seq', (SELECT MAX(id)
  from "levels"));
